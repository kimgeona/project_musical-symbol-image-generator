# Musical-Symbol-Image-Generator (v.2.0.1)

"MSIG(Musical-Symbol-Image-Generator)"는 머신러닝 학습을 위한 악상 기호 데이터셋 생성기입니다. 해당 프로그램은 악상기호들의 모든 조합을 계산하여 악상기호 이미지들과 이에 대한 레이블을 제공하는 것을 목표로 하고 있습니다.

현재 프로그램은 실행파일로 제공되지 않고 원본 소스코드로 제공됩니다. 사용하시기 위해서는 이를 직접 빌드 해서 사용하셔야 합니다. 다음 절에서 빌드 방법을 설명합니다.

>지원 환경 : MacOS(Xcode), Windows(VisualStudio2022)

## 업데이트 내용 (v.2.0.0)
* \_\_rule\_\_.txt 파일을 이용하여 더 복잡한 악상기호 조합 규칙 구성이 가능해짐.
* 데이터셋 생성시 필요한 메모리 사용량 대폭 감소.
