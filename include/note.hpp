...
private:
    ...
    // note 데이터 저장 벡터
    vector<bool>    buffer_type;    // 음표 종류 : note, beat, bar, ...
...
public:
    ...
    // note 데이터 추가 함수
    void add_type(bool type);
    void add_pitch(string pitch);
    void add_beat(string beat);
    void add_clef(string clef);
    void add_time(string time);
    void add(bool type, string pitch, string beat, string clef, string time);
    
    // note 데이터 오류 확인 함수
    bool check_bufer_count();   // buffer에 저장된 갯수 서로 올바른지 확인
