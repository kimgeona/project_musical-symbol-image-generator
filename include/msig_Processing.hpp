...
{
public:
    static cv::Mat mat_attach   (const cv::Mat& img, const cv::Mat& img_sub, int x, int y, bool trim=true);
    static cv::Mat mat_rotate   (const cv::Mat& img, double& degree, int& cx, int& cy);
    static cv::Mat mat_scale    (const cv::Mat& img, double& scale, int& cx, int& cy);
    static cv::Mat mat_symmetry (const cv::Mat& img, std::string symmetry);
    static cv::Mat remove_padding(const cv::Mat& img, int& center_x, int& center_y);
};
